...
vpn_status_panel() {
    ...
    local ver conn proto lock
    ver=$(_vpn_mullvad version 2>/dev/null | head -1)
    conn=$(_vpn_mullvad status 2>/dev/null | head -1)
    proto=$(_vpn_mullvad relay get 2>/dev/null | grep -iE 'tunnel|protocol' | head -1)
    lock=$(_vpn_mullvad lockdown-mode get 2>/dev/null | head -1)
    log_table \
        "Cliente" "${ver:-?}" \
        "Conexión" "${conn:-?}" \
        "Protocolo" "${proto:-?}" \
